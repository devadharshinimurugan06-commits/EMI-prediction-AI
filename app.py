...
# =========================================================
    ...
    st.subheader("Create: Add a Manual Record")

with st.form("manual_add"):
    manual_salary = st.number_input(
        "Monthly Salary",
        5000,
        500000,
        50000
    ...
    if submitted:
        ...
        if os.path.exists(log_file):
            ...
            # Create a new row with the EXACT same columns
            # as the existing prediction records
            new_row = pd.DataFrame(
                {column: [pd.NA] for column in records.columns}
            )

            # Put values into their correct columns
            new_row.loc[0, 'monthly_salary'] = manual_salary
            new_row.loc[0, 'requested_amount'] = manual_amount

            # Append the complete row
            new_row.to_csv(
                log_file,
                mode='a',
                header=False,
            ...
            # If no prediction record exists yet,
            # create the first record with the required columns
            new_row = pd.DataFrame([{
                'age': pd.NA,
                'gender': pd.NA,
                'marital_status': pd.NA,
                'education': pd.NA,
                'monthly_salary': manual_salary,
                'employment_type': pd.NA,
                'years_of_employment': pd.NA,
                'company_type': pd.NA,
                'house_type': pd.NA,
                'monthly_rent': pd.NA,
                'family_size': pd.NA,
                ...
                'school_fees': pd.NA,
                'college_fees': pd.NA,
                'travel_expenses': pd.NA,
                'groceries_utilities': pd.NA,
                'other_monthly_expenses': pd.NA,
                'existing_loans': pd.NA,
                'current_emi_amount': pd.NA,
                'credit_score': pd.NA,
                'bank_balance': pd.NA,
                'emergency_fund': pd.NA,
                'emi_scenario': pd.NA,
                'requested_amount': manual_amount,
                'requested_tenure': pd.NA,
                'predicted_eligibility': pd.NA,
                'predicted_max_emi': pd.NA
            }])
