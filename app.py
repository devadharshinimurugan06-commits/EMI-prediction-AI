...
# =========================================================
    ...
    with st.form("manual_add"):
        manual_salary = st.number_input("Monthly Salary", 5000, 500000, 50000)
        manual_amount = st.number_input("Requested Amount", 5000, 2000000, 100000)
        submitted = st.form_submit_button("Add Record")
        if submitted:
            new_row = pd.DataFrame([{'monthly_salary': manual_salary, 'requested_amount': manual_amount}])
            if os.path.exists(log_file):
